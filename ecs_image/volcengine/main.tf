...
resource "volcenginecc_vpc_security_group" "SecurityGroupDemo" {
  ...
  project_name        = var.project_name
  tags                = var.tags
  depends_on          = [volcenginecc_vpc_subnet.SubnetDemo]
}

resource "volcenginecc_ecs_instance" "tf2" {
  ...
  primary_network_interface = {
    security_group_ids = [volcenginecc_vpc_security_group.SecurityGroupDemo.id]
...
resource "volcenginecc_ecs_image" "image_demo" {
  image_name   = "tf2-test-image"
  description  = "tf2-test"
  instance_id  = volcenginecc_ecs_instance.tf2.id
  project_name = var.project_name
  tags         = var.tags
}
